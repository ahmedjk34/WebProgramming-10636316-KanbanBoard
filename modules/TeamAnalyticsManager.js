...
class TeamAnalyticsManager {
  constructor(dependencies = {}) {
    ...
    console.log("🔧 TeamAnalyticsManager initialized with dependencies:", Object.keys(dependencies));
  }
  ...
  async loadTeamAnalytics(teamId, options = {}) {
    ...
    const { days = 30, includeMembers = true, includeProjects = true } = options;
  ...
  async getProductivityHeatmap(teamId, days = 30) {
    ...
    try {
      const response = await this.apiManager.getProductivityHeatmap(teamId, days);
      return response.success ? response.data : this.getMockProductivityHeatmap();
    } catch (error) {
  ...
  getMockActivityTimeline() {
    ...
    return {
      timeline,
      summary: {
        total_tasks_created: timeline.reduce((sum, day) => sum + day.tasks_created, 0),
        total_tasks_completed: timeline.reduce((sum, day) => sum + day.tasks_completed, 0),
        average_daily_activity: timeline.reduce((sum, day) => sum + day.tasks_updated, 0) / days,
  ...
  getMockProductivityHeatmap() {
    ...
    for (let dayIndex = 0; dayIndex < days.length; dayIndex++) {
      ...
      for (let hourIndex = 0; hourIndex < hours.length; hourIndex++) {
        ...
        if (dayIndex < 5) { // Weekdays
          if (hourIndex >= 1 && hourIndex <= 3) { // 9AM - 3PM
            value = Math.floor(Math.random() * 40) + 60; // High productivity
          } else if (hourIndex === 0 || hourIndex === 4) { // 6AM, 6PM
    ...
    return {
      ...
      summary: {
        ...
        average_productivity: heatmap.flat().reduce((sum, cell) => sum + cell.value, 0) / heatmap.flat().length,
  ...
  updateTeamStatsCards() {
    ...
    this.updateStatCard('completed-tasks', stats.completed_tasks, 'Completed Tasks');
    this.updateStatCard('completion-rate', `${stats.completion_rate}%`, 'Completion Rate');
    this.updateStatCard('productivity-score', `${stats.productivity_score}%`, 'Productivity Score');
    this.updateStatCard('team-velocity', stats.team_velocity, 'Team Velocity');
    this.updateStatCard('collaboration-score', `${stats.collaboration_score}%`, 'Collaboration Score');
  }
  ...
  updateMemberPerformanceChart() {
    ...
    this.createOrUpdateChart('member-performance', chartContainer, 'bar', chartData);
  }
  ...
  updateProjectMetricsChart() {
    ...
    // Create or update chart
    this.createOrUpdateChart('project-metrics', chartContainer, 'bar', chartData);
  }
  ...
  updateActivityTimelineChart() {
    ...
    // Create or update chart
    this.createOrUpdateChart('activity-timeline', chartContainer, 'line', chartData);
  }
      ...
      html += `
        <div class="bar-group">
          ...
          <div class="bar-container">
            <div class="bar bar-1" style="height: ${value1}%; background-color: ${data.datasets[0].backgroundColor}"></div>
            ${data.datasets[1] ? `<div class="bar bar-2" style="height: ${value2}%; background-color: ${data.datasets[1].backgroundColor}"></div>` : ''}
          </div>
  ...
  downloadCSV(data, filename) {
    ...
    if (data.analytics.memberPerformance) {
      ...
      let csv = 'Name,Role,Tasks Completed,Completion Rate,Productivity Score\n';
